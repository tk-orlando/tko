									...
									<div class="themify_builder_input"<?php echo ( 'wp_editor' == $option['type'] ) ? ' style="width:100%;"' : ''; ?>>
										<?php if( $option['type'] == 'text' ): ?>
											...
											<?php if( isset($option['colorpicker']) && $option['colorpicker'] == true ) : ?>
												...
												<input type="text" class="<?php echo esc_attr( $option['class'] ); ?> colordisplay" <?php echo themify_builder_get_binding_data( $option ); ?> />
											...
											<?php else : ?>
												<input name="<?php echo esc_attr( $option['id'] ); ?>" class="<?php echo esc_attr( $option['class'] ); ?> tfb_lb_option_child <?php echo isset( $add_class ) ? esc_attr( $add_class ) : ''; ?>" type="text" data-input-id="<?php echo esc_attr( $option['id'] ); ?>" />
										...
										<?php elseif( 'radio' == $option['type'] ): ?>
											<div data-input-id="<?php echo esc_attr( $option['id'] ); ?>" class="tfb_lb_option_child tf-radio-choice">
												<?php foreach( $option['options'] as $k => $v ): ?>
													...
													<label for="<?php echo esc_attr( $option['id'] .'_'. $k ); ?>" class="pad-right themify-builder-radio-dnd-label"><?php echo wp_kses_post( $k ); ?></label>
												<?php endforeach; ?>
											</div>
										<?php endif; // endif radio input ?>
				...
				<div class="themify_builder_input<?php echo isset($field['pushed']) && $field['pushed'] != '' ? ' '.$field['pushed'] : ''; ?>">
					...
					<?php elseif( 'checkbox' == $field['type'] ): ?>
						...
						<?php foreach( $field['options'] as $opt): ?>
							...
							?>
							<input id="<?php echo esc_attr( $field['id'] . '_' . $opt['name'] ); ?>" name="<?php echo esc_attr( $field['id'] ); ?>[]" type="checkbox" class="tf-checkbox" value="<?php echo esc_attr( $opt['name'] ); ?>" <?php echo $checkbox_checked; ?> />
					...
					<textarea id="<?php echo esc_attr( $field['id'] ); ?>" name="<?php echo esc_attr( $field['id'] ); ?>" class="<?php echo esc_attr( $field['class'] ); ?> tfb_lb_option" row="3" type="text" <?php echo themify_builder_get_binding_data( $field ); ?>></textarea>
		...
		switch ($styling['type']) {
			...
			case 'checkbox':
				...
				<?php foreach( $styling['options'] as $opt): ?>
					...
					?>
					<input id="<?php echo esc_attr( $styling['id'] . '_' . $opt['name'] ); ?>" name="<?php echo esc_attr( $styling['id'] ); ?>" type="checkbox" class="tfb_lb_option tf-checkbox" value="<?php echo esc_attr( $opt['name'] ); ?>" <?php echo $checkbox_checked; ?> />
