            ...
            <div class="inside">
                ...
                foreach($write_panel['options'] as $meta_box):
					...
					if($meta_box['type'] == 'image'){ ?>
						...
						<div class="themify_field_row clearfix featimgcontrol <?php echo esc_attr( $toggle_class ); ?>" <?php echo esc_attr( $ext_attr ); ?>>
					...
					///////////////////////////////////////////
						...
						<input type="hidden" name="<?php echo esc_attr( $meta_box['name'] . '_noncename' ); ?>" id="<?php echo esc_attr( $meta_box['name'] . '_noncename' ); ?>" value="<?php echo wp_create_nonce( plugin_basename(__FILE__) ); ?>" />
                        <div class="themify_field_row clearfix <?php echo esc_attr( $toggle_class ); ?>" <?php echo esc_attr( $ext_attr ); ?>>
					...
					///////////////////////////////////////////
						...
						<input type="hidden" name="<?php echo esc_attr( $meta_box['name'] . '_noncename' ); ?>" id="<?php echo esc_attr( $meta_box['name'] . '_noncename' ); ?>" value="<?php echo wp_create_nonce( plugin_basename(__FILE__) ); ?>" />
						<div class="custom-post-meta-group themify_field_row clearfix <?php echo esc_attr( $toggle_class ); ?>" <?php echo esc_attr( $ext_attr ); ?>>
...
function themify_meta_field_date( $args, $call_before_after = true, $echo = true ) {
	...
	esc_attr( $meta_box['name'] ), esc_attr( $meta_box['name'] ), esc_attr( $meta_value? $meta_value : $default ), 'clear-'.esc_attr( $meta_box['name'] ), esc_attr( $pick ), esc_attr( $close ), esc_attr( $date_format ), esc_attr( $time_format ), esc_attr( $timeseparator ),
...
function themify_meta_field_separator($args, $call_before_after = true, $echo = true){
	...
	$html = '<div class="themify_field_row clearfix ' . esc_attr( $toggle_class ) . '" ' . esc_attr( $ext_attr ) . '>'.$html.'</div>';
...
function themify_get_image( $args ) {
	...
	if( in_the_loop() && ( ! isset( $ignore ) || !$ignore ) ) {
		if( is_home() || is_archive() || is_search() ) {
			if ( isset( $data["setting-{$setting}_height"] ) && $data["setting-{$setting}_height"] != '' && $setting != '' ) {
				...
				if ( ( 'no' == themify_get( 'setting-img_settings_vertical_crop_option' ) ) || 0 == $cfheight ) {
					$height = '';
				} elseif( $cfheight ) {
					$height = $cfheight;
		...
		if ( 'no' == themify_get( 'setting-img_settings_vertical_crop_option') ) {
			$height = '';
		} else {
			if( isset( $h ) && $h != '' ) {
				$height = $h;
	...
	$p_height = isset( $height ) && $height != '' ? 'height="' . esc_attr( $height ) . '"' : '';
	$p_width = isset( $width ) && $width != '' ? 'width="' . esc_attr( $width ) . '"' : '';
	...
	if ( isset( $src ) && $src != '' && $src ) {
		...
		if( ! isset($data['setting-img_settings_use']) || '' == $data['setting-img_settings_use'] || !$data['setting-img_settings_use'] ){
			...
			if( '' == $feature_size || !has_post_thumbnail() ){
				...
				if( ! isset( $ignore ) || !$ignore ){
					$p_width  = 'width="' . esc_attr( $themify->width ) . '"';
					$p_height = 'height="' . esc_attr( $themify->height ) . '"';
				}

				// For singular views
				if ( is_singular() && ( ! isset( $ignore ) || ! $ignore ) ) {
					if ( isset( $feature_width ) ) {
						$p_width = 'width="' . esc_attr( $feature_width ) . '"';
					} elseif ( isset( $data["setting-{$setting}_width"] ) && '' != $data["setting-{$setting}_width"] && '' != $setting ) {
						$p_width = 'width="' . esc_attr( $data["setting-{$setting}_width"] ) . '"';
					}

					if ( isset( $feature_height ) ) {
						$p_height = 'height="' . esc_attr( $feature_height ) . '"';
					} elseif ( isset( $data["setting-{$setting}_height"] ) && '' != $data["setting-{$setting}_height"] && '' != $setting ) {
						$p_height = 'height="' . esc_attr( $data["setting-{$setting}_height"] ) . '"';
					}
				}
				...
				if( ( ! isset( $ignore ) || !$ignore ) && isset( $post_query_category ) ){
					...
					if( '' != $queryw ){
						$p_width = 'width="' . esc_attr( $queryw ) . '"';
					}
					if( '' != $queryh ){
						$p_height = 'height="' . esc_attr( $queryh ) . '"';
					}
				}
				...
				if( ( ! isset( $ignore ) || !$ignore ) && $iscpt ){
					...
					if ( '' != $cptw ) {
						$p_width = 'width="' . esc_attr( $cptw ) . '"';
					}
					if ( '' != $cpth ) {
						$p_height = 'height="' . esc_attr( $cpth ) . '"';
					}
				}
		...
		if ( ! empty( $img_url ) ) {
			if ( isset( $urlonly ) ) {
				...
				return stripslashes( $before ) . '<img src="'. esc_url( $img_url ) . '" alt="' . esc_attr( $alt ) . '"' . " $class $id $p_width $p_height />" . stripslashes( $after );
			}
		}
	}
	return '';
...
function themify_area_design( $key = 'header', $args = array() ) {
	...
	$header = themify_check( $args['setting'] ) ? themify_get( $args['setting'] ) : $args['default'];
	if ( is_singular() ) {
