...
function emg_customposttype_image_box() {
	...
	add_meta_box( 'emgaffdiv', __( 'Join Now' ), 'easmedia_aff_metabox', 'easymediagallery', 'side', 'default' );
	add_meta_box( 'emgratediv', __( 'Rate Us' ), 'easmedia_rate_metabox', 'easymediagallery', 'side', 'default' );
		}
	...
	add_action( "admin_head", 'emg_load_ckeditor' );
			...
			function emg_load_ckeditor() {
				...
				if ( get_post_type( get_the_ID() ) == 'easymediagallery' ) {
					...
					//wp_enqueue_script( 'easymedia-ckeditor' ); @since 1.5.1.0
					...
					wp_enqueue_script( 'easymedia-uploader', plugins_url( 'functions/uploader.js' , __FILE__ ) );
					...
					wp_enqueue_style( 'jquery-spagination-css' );
					wp_enqueue_script( 'jquery-spagination-js' );
					wp_enqueue_script( 'jquery-notify-js' );
					wp_enqueue_script( 'meta-lazy', plugins_url( 'js/jquery/jquery.lazyload.min.js' , __FILE__ ) );
					
    				wp_localize_script( 'easymedia-uploader', 'emguploader' , array(
					'ispost_id' => get_the_ID()
						) );
					
					// Check timthumb compatibility, we put the call here because we want this function only run in media editor page
					// @since 1.5.1.7
					if( easy_get_option( 'easymedia_disen_tthumb' ) == '1' ) {
						emg_metabox_timthumb_check( dirname(__FILE__).'/class/cache/'.time().'.txt');
						}			
		...
		@media only screen and (min-width: 1150px) {	
		    #submitdiv.fixed { position: fixed; top: 55px; right: 20px; width: 280px; z-index: 999; }
		}	
		</style>
		<script>
		jQuery(document).ready(function($) {
		    var snpprevPosition = jQuery('#submitdiv').offset();
		    jQuery(window).scroll(function(){
				<?php if ( easy_get_option( 'easymedia_disen_dasnews' ) == '1' ) { ?>
			     if(jQuery(window).scrollTop() > snpprevPosition.top * 6)
				 <?php } else { ?>
				 if(jQuery(window).scrollTop() > snpprevPosition.top)
				 <?php } ?>
			    {
				jQuery('#submitdiv').addClass('fixed');
			    } 
			    else 
			    {
				jQuery('#submitdiv').removeClass('fixed');
			    }    
		    });
		});
		</script>
          ...
          <script type="text/javascript">
			/*<![CDATA[*/
			jQuery(document).ready(function($) {
				jQuery('#wp-easmedia_metabox_shordesc-media-buttons a').not('#insert-media-button').hide();			
				});
  	/*]]>*/
		</script>    
			<?php
...
function easmedia_create_meta_box( $post, $meta_box )
{
	...
	foreach ( $meta_box['fields'] as $field ){
		...
		switch( $field['type'] ){	
			case 'text':
				echo '<td><input type="text" name="easmedia_meta['. $field['id'] .']" id="'. $field['id'] .'" value="'. ($meta ? stripslashes( esc_html( esc_js( $meta ) ) ) : $field['std']) .'" size="30" /></td>';
				break;	
...
<img id="videothumbnailprv" style="display:none;" src="http://placehold.it/300x190" height="190" width="300"/></div>
			...
			$images = get_post_meta( $post->ID, 'easmedia_metabox_media_gallery', true ); 
				...
				echo '<td></tr>
            <div id="g-img_list" style="display: none;"></div><input type="hidden" name="easmedia_meta['. $field['gallid'] .']" value="gallery-'. $post->ID .'" />	
          </div><span class="emggtips">Tips:</span><ul class="emggtipslist"><li>You can use (<b>Ctrl + Click</b>) or (<b>Shift + Click</b>) on each image to select multiple images at once.</li><li>You can drag and drop images to re-order.</li><li>You also can click on image to edit title/subtitle but don\'t forget to switch ON <strong>Use information of each image</strong> option first.</li></ul><div style="clear: both; height: auto; padding: 20px; border-bottom: 1px solid #DDD; border-top: 1px solid #DDD; margin-top:5px;margin-bottom:5px;"><span class="emg_add_images">Insert / Select Image</span></div>	
		  	<div class="emglistloader">
            <div id="g-img-wrap" style="display: none;">
				...
				if ( is_array( $images ) ) {
					foreach( $images as $img_id ) {
						...
						<li class="gallitem" id="currentimg-'.$img_id.'">
							...
							<img height="90" width="90" class="isshort" data-original="'.$galleryurl.'" />
            ...
            </div></div><div style="width:100%; text-align:center; margin-top: 25px;"><div style="margin:auto;display:none;" id="pagination"></div><span id="isnoimg" style="display: none;">No image(s) selected...</span><span id="emgexpcol" data-pag="off">Show all Images</span></div></td>';
				break;
			...
			case 'textareackeditor':
				...
				echo '<td>';	
				wp_editor( ($meta ? $meta : $field['std']) , 'easmedia_metabox_shordesc', array(
				'textarea_name' => 'easmedia_meta['. $field['id'] .']',
				'media_buttons' => true,
				'textarea_rows' => 11,
				'wpautop' => true
				 ) );
...
function easmedia_metabox_work(){
	...
	// VIDEO METABOX
		...
		'fields' => array(
			...
			array(
					...
					'name' => __( 'Video URL (Youtube/Vimeo/MP4/WMV)', 'easmedia' ),
					'desc' => __( 'Use this field to embed video from Youtube, Vimeo, MP4 video type or WMV video type.', 'easmedia' ),
	...
	// GOOGLE MAPS METABOX
		...
		'title' =>  __( 'Options', 'easmedia' ),
		'description' => __( 'Paste Google Maps, Street View or Bussiness View URL to field below.', 'easmedia' ),
		'page' => 'easymediagallery',
		'context' => 'normal',
		'priority' => 'default',
		'fields' => array(
			...
			array(
					'name' => __( 'Lightbox Size', 'easmedia' ),
					'desc' => __( 'If ON, lightbox size will use the default settings on the control panel.', 'easmedia' ),
    ...
    $meta_box = array(
		...
		'fields' => array(
			...
			array(
					...
					'desc' => __( 'Enter description for your media. NOTE : This description will appear when you use EMG Lightbox only. For other lightbox such as Foobox, Lightbox 2, PrettyPhoto only use Media title and sub title.', 'easmedia' ),
